...
global OfficerChoices, FacultyChoices, PresidentChoices
...
btn_exit.grid(padx=50, row = 10, columnspan =12)
...
########################################################
        ...
        if confirmed == True:
            ...
            lbl_Faculty = Label(Receipt,text="Faculty Officer Votes:\n"+str(self.Choices[1])).grid(row=2,column=2)
            lbl_President = Label(Receipt,text="President Votes:\n"+str(self.Choices[2])).grid(row=2,column=3)
            Receipt.mainloop()
            Home.deiconify()
        else:
            self.roleCount+=1
